...
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.{i686,${arch}})" install
}
...
if [[ ${EUID} -ne 0 ]]; then
    echo "Run Pake Root Gan!"
    _usage 1
fi

if [[ ${arch} != x86_64 ]]; then
    echo "Script ini khusus x86_64"
    _usage 1
fi
